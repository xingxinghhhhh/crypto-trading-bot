...
def main() -> None:
    ...
    operations_handoff_consumer_projection_receipt_parser = subparsers.add_parser(
    ...
    segment_evidence_parser = subparsers.add_parser(
        "materialize-prospective-direct-1h-segment-evidence"
    )
    segment_evidence_parser.add_argument("--segment-admission", required=True)
    segment_evidence_parser.add_argument("--capture-report", default=None)
    segment_evidence_parser.add_argument(
        "--config", default="config.prospective-direct-1h-segment-evidence.example.yaml"
    )
    segment_evidence_parser.add_argument(
    ...
    if args.command == "materialize-verified-current-operations-handoff-projection-receipt":
        try:
            receipt_result = (
                materialize_governance_fresh_current_operations_handoff_projection_provenance_receipt(
                    args.projection,
            ...
            print(
                f"prospective_evidence_operations_handoff_consumer_projection_provenance_receipt_failed: {exc}",
                file=sys.stderr,
            )
            raise SystemExit(2) from exc
    ...
    if args.command == "materialize-prospective-direct-1h-segment-evidence":
        try:
            segment_evidence_result = materialize_prospective_direct_1h_segment_evidence(
                args.segment_admission,
                args.config,
                args.output_dir,
                args.capture_report,
            )
        except (FileNotFoundError, OSError, ValueError, MarketDataError) as exc:
            print(f"prospective_direct_1h_segment_evidence_failed: {exc}", file=sys.stderr)
            raise SystemExit(2) from exc
